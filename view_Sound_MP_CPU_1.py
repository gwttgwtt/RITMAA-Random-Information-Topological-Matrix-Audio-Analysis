...
def song_sf_vector(path, seg_size, min_segments):
    y, sr = read_mono(path)
    ...
    v = np.log1p(ratio)
    v = sanitize_vector(v, label=Path(path).name)
...
def _song_sf_worker(args):
    ...
    v, sr = song_sf_vector(Path(path_str), seg_size, min_segments)
...
def build_matrix(files, seg_size, status_callback=None, n_workers=1):
    t0 = time.time()
    ...
    if n_workers <= 1:
        for i, p in enumerate(files, 1):
            ...
            v, _ = song_sf_vector(p, seg_size, min_segments)
            X[i - 1] = v

            log_memory("after song")
            gc.collect()
    else:
        tasks = [(i, str(p), seg_size, min_segments) for i, p in enumerate(files)]
...
def mp_reconstruct(X, n_modes, use_gpu=False):
    ...
    if T <= N:
        raise RuntimeError(f"MP boundary error: T={T}, N={N}. Need T > N.")
...
class MPOfflineRenderGUI:
    def __init__(self, root):
        ...
        tk.Button(top, text="Select folder", command=self.select_folder).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(top, text="Analyze MP", command=self.analyze).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(top, text="Render filtered files", command=self.render).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(top, text="Render archetype collage", command=self.render_archetype_collage).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(top, text="Export K1-16 PDF", command=self.export_entropy_maps_pdf).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(top, text="Save CSV", command=self.save_csv).pack(side=tk.LEFT, padx=5, pady=5)

        self.use_gpu = tk.BooleanVar(value=False)
        tk.Label(top, text="CPU-only mode").pack(side=tk.LEFT, padx=10)

        tk.Label(top, text="CPU workers").pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(top, text="Segment").pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(top, text="MP modes").pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(top, text="Dry %").pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(top, text="Wet %").pack(side=tk.LEFT, padx=5)
    ...
    def analyze(self):
        ...
        try:
            ...
            self.set_status("Building Shannon/Fisher matrix...")
            ...
            self.set_status(
                f"Done. Segments={self.min_segments}, "
    ...
    def update_tables_and_plot(self, seg, k):
        ...
        # ВИЗУАЛИЗАЦИЯТА ВЕЧЕ Е ВЪРХУ Xrec,
        # за да се променя при смяна на MP modes.
        if self.Xrec is not None:
    ...
    def render(self):
        try:
            ...
            out_dir = self.folder.parent / f"{self.folder.name}_MP_LOWRANK_AUDIO_K{k}_seg{seg}_D{int(dry*100)}_W{int(wet*100)}"
    ...
    def draw(self, ranked_names, ranked_heat, evals, lam_minus, lam_plus, n_signal, seg, k):
        ...
        self.ax_heat.set_title(f"MP reconstructed Shannon/Fisher field — seg={seg}, modes={k}")
        self.ax_heat.set_xlabel("Segment index")
        self.ax_heat.set_ylabel("Song rank")
    ...
    def update_segment_view(self, redraw=True):
        ...
        self.ax_eig.set_title(
            f"Exact vertical-column eigenvalues | clicked segment {center} | all songs | modes>{lam_plus:.3g}: {n_signal}",
            fontsize=10
        )
        ...
        try:
            self.ax_density.bar(np.arange(1, len(vcol) + 1), vcol, width=0.85, alpha=0.85)
            ...
            self.ax_density.set_xlabel("Song index in current matrix order")
            self.ax_density.set_ylabel("z-scored Shannon/Fisher value")
        ...
        if self.heat_marker is not None:
            try:
                self.heat_marker.remove()
            except Exception:
                pass
            self.heat_marker = None

        if self.heat_window_patch is not None:
            try:
                self.heat_window_patch.remove()
            except Exception:
                pass
            self.heat_window_patch = None

        self.heat_marker = self.ax_heat.axvline(center, color="red", linewidth=2.2)

        # Optional selected row marker from heatmap click.
        if self.heat_row_marker is not None:
            try:
                self.heat_row_marker.remove()
            except Exception:
                pass
            self.heat_row_marker = None
        if self.selected_rank_row is not None:
            self.heat_row_marker = self.ax_heat.axhline(int(self.selected_rank_row), color="red", linewidth=1.4, alpha=0.95)

        # translucent local covariance window, to show what columns generate the local spectrum
        self.heat_window_patch = self.ax_heat.axvspan(left, max(left, right - 1), color="red", alpha=0.10)
    ...
    def on_heatmap_click(self, event):
        """Click on heatmap: select song row + segment column and draw beat chart."""
        ...
        try:
            ...
            self.plot_selected_segment_beat(file_idx, seg_idx, rank_row)
            self.update_segment_view(redraw=False)
            self.canvas.draw_idle()
        except Exception as e:
            traceback.print_exc()
    ...
    def refresh_selected_audio_view(self, redraw=True):
        """Redraw lower original/filtered waveform for current selected song/segment."""
    ...
    def save_csv(self):
        ...
        self.df_rank.to_csv(f"mp_offline_ranking_K{k}_seg{seg}.csv", index=False)
        self.df_eig.to_csv(f"mp_offline_eigenvalues_K{k}_seg{seg}.csv", index=False)
